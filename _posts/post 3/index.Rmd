---
title: "Zomato's best restaurants in Melbourne"
subtitle: "This project explores the top rated restaurants in Melbourne based off zomato ratings. This project with written for ETC5513: Collaborative and Reproducible Practices "
description: |
  This project explores the top rated restaurants in Melbourne based off zomato ratings. This project with written for ETC5513: Collaborative and Reproducible Practices
author:
  - name: Vinny Vu
...
Access the git repository for this project via the following [link](https://github.com/dedi0003/assignment4_etc5513)  
The Full PDF report can be found via the following [link](https://github.com/dedi0003/assignment4_etc5513/blob/master/index.pdf)
...
melb_top100_filter %>%
  select(name, average_cost_for_two,aggregate_rating, price_range) %>%
  ...
  ggplot(aes(x = average_cost_for_two,
             y = aggregate_rating)) +
  geom_point(aes(colour = price_range)) +
...
```{r best-restaurant, echo=FALSE, fig.cap="The Very Best Restaurants in Melbourne According to Zomato Rating"}
melb_best_resto %>%
  datatable() %>%
  DT::formatStyle(columns = c(0,1,2,3,4,5,6), fontSize = '60%')
```


From table \@ref(fig:best-restaurant) above, there are twelve restaurants which have minimum rating 4.8 out of 5 hence can be described as the very best restaurants in Melbourne. The prices, cuisines and locations in the top list vary but with some patterns that will be further explored in the following section.
...
```{r map-icon, fig.cap="Sushi Restaurants are Dominating Top List", out.width= "70%", fig.align='center'}
...
This fact support the opinion that "Japanese food is still growing since UNESCO declared Japanese cuisine to be an Intangible Cultural Heritage in 2013" @sushi.
...
cuisines_melb %>%
  ggplot(aes(x = n, y = reorder(Types_of_cuisine, n),fill = Types_of_cuisine))+
           geom_bar(stat = "identity")+
  xlab("Count") +
  ylab("") +
  theme_bw() +
...
cuisines_melb_ratings %>%
  ggplot(aes(x = Total_Votes, y = reorder(Types_of_cuisine,Total_Votes), fill = Types_of_cuisine))+
           geom_bar(stat = "identity") +
  labs(x = "Total Votes",
      y = "") +
  theme_bw() +
...
```{r  include = FALSE}
kable(japanese_restro, caption = "Top 10 voted Japanese restaurants in Melbourne", booktabs = T) %>%
  kable_styling(latex_options = c("striped", "hold_position"))
```
```{r top10-Jap, echo=FALSE, fig.cap= "Top 10 voted Japanese restaurants in Melbourne"}
japanese_restro %>%
  datatable()%>%
  DT::formatStyle(columns = c(0,1,2,3,4,5,6), fontSize = '60%')
...
In Table \@ref(fig:top10-Jap) we can look at the top 10 popular Japanese restaurants in Melbourne along with their location and rating. 'Don-Don' is a very popular Japanese restaurant with 3168 votes. 'Shira Nui' is the highest rated restaurant with an aggregated rating of 4.8 out of 5.
...
In this section we will be looking at restaurants with gluten free food.    
...
```{r rest-gluten-free, echo = FALSE, fig.cap= "Top Options"}
glute %>%
  datatable()%>%
  DT::formatStyle(columns = c(0,1,2,3,4,5,6), fontSize = '60%')
```
...
The Table \@ref(fig:rest-gluten-free) shows the name of the restaurants in Melbourne, their locality, number of reviews received, aggregate ratings, photo count and how much it costs for one persons gluten free meal on average. 
...
The graph Figure \@ref(fig:instagrammable-plot) visualizes photo count against review counts for each restaurants potentially highlighting how "instagrammable" a potential restaurant is. Lune Croissanterie has the best review rating with a high photo count of 1813. While, The Hardware Societe has the highest photo count at 4224, with a good rating of 4.6.
...
* Melbourne has a strong multicultural cuisines options for people to choose from. Japanese being the most popular having the highest count of restaurants. Popularity of Cafes are also high.
* Most of the very best restaurants in Melbourne is located near CBD. 
* Melbourne has quite a few good restaurants that serve gluten free food with a high review count. Most of the the same restaurants have a high photo count suggesting they are also "instagramable" 
